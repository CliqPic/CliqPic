  ...
  class SearchHashtagsJob < ApplicationJob
    ...
    def perform(event_id, user_id, options={})
      ...
      possible_images.each do |p_image|
        ...
        most_recent_image_id = p_image["id"]
      end

      total_new = (options[:total_new] || 0) + total_new

      puts "Possible images is #{most_recent_image_id} and #{total_new}"
      if total_new < 100 and most_recent_image_id
        puts "performing a new guy"
        # If we got a full page and the last image we grabbed was new, queue a job
        # to grab the next page
        self.class.perform_later(event_id, user_id, options.merge(max_id: most_recent_image_id, total_new: total_new))
      end
